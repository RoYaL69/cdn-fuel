if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    RegisterNetEvent('cdn-fuel:station:client:initiatefuelpickup', function(amountBought, finalReserveAmountAfterPurchase, location)
        if amountBought and finalReserveAmountAfterPurchase and location then
            ...
            if SpawnPickupVehicles() then
                QBCore.Functions.Notify(locale("fuel_order_ready"), 'success')
                                                            ...
                                                            if IsControlJustReleased(2, 38) then
                                                                ...
                                                                if distBetweenTruckAndTrailer > 10.0 then
                                                                    ...
                                                                    if not stopNotifyTemp then
                                                                        QBCore.Functions.Notify(locale("trailer_too_far"), 'error', 7500)
                                                                    end
                                                                    stopNotifyTemp = true
                                                                    Wait(1000)
                                                ...
                                                locationSwapHandler = AddEventHandler('cdn-fuel:stations:updatelocation', function(location)
                                                    if location == nil or location ~= ReservePickupData.location then
                                                        ...
                                                        if Config.Ox.DrawText then
                                                            ...
                                                            exports[Config.Core]:DrawText(locale("draw_text_fuel_dropoff"), 'left')
                                                        end
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchaselocation', function(data)
        ...
        elseif IsOwned then
            QBCore.Functions.Notify(locale("station_already_owned"), 'error', 7500)
        end
    end)
    ...
    RegisterNetEvent('cdn-fuel:stations:client:sellstation', function(data)
        ...
        QBCore.Functions.TriggerCallback('cdn-fuel:server:isowner', function(result)
            if result then
                ...
                QBCore.Functions.Notify(locale("station_not_owner"), 'error', 7500)
        ...
        if CanSell then
            ...
            QBCore.Functions.Notify(locale("station_cannot_sell"), 'error', 7500)
        end
    end)
        ...
        if CanOpen then
            ...
            if Config.Ox.Menu then
                ...
                exports['qb-menu']:openMenu({
                    {
                        header = locale("menu_station_reserves_header")..Config.GasStations[location].label,
                        isMenuHeader = true,
                        icon = "fas fa-gas-pump",
                    },
                    {
                        header = locale("menu_station_reserves_purchase_header")..price,
                        txt = locale("menu_station_reserves_purchase_footer")..price.."!",
                        icon = "fas fa-usd",
                        params = {
                            event = "cdn-fuel:stations:client:buyreserves",
                            args = {
                    ...
                    },
                    {
                        ...
                        txt = locale("menu_station_reserves_cancel_footer"),
                        icon = "fas fa-times-circle",
                        params = {
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if reserves then
                    ...
                    if not reservesAmount then QBCore.Functions.Notify(locale("station_amount_invalid"), 'error', 7500) return end
                    Reservebuyamount = tonumber(reservesAmount)
                    if Reservebuyamount < 1 then QBCore.Functions.Notify(locale("station_more_than_one"), 'error', 7500) return end
                    if (Reservebuyamount + Currentreserveamount) > Config.MaxFuelReserves then
                        QBCore.Functions.Notify(locale("station_reserve_cannot_fit"), "error")
                    else
                        if math.ceil(GlobalTax(Reservebuyamount * Config.FuelReservesPrice) + (Reservebuyamount * Config.FuelReservesPrice)) <= bankmoney then
                            ...
                            QBCore.Functions.Notify(locale("not_enough_money_in_bank"), 'error', 7500)
                        end
                    end
                end
            else
                local reserves = exports['qb-input']:ShowInput({
                    ...
                    inputs = { {
                        ...
                        isRequired = true,
                        name = 'amount',
                        text = locale("input_purchase_reserves_text")
                    }}
                })
                if reserves then
                    if Config.FuelDebug then print("Attempting to buy reserves!") end
                    ...
                    if not reserves.amount then QBCore.Functions.Notify(locale("station_amount_invalid"), 'error', 7500) return end
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if fuelprice then
                    ...
                    Wait(100)
                    if not fuelPrice then QBCore.Functions.Notify(locale("station_amount_invalid"), 'error', 7500) return end
                    NewFuelPrice = tonumber(fuelPrice)
                    if NewFuelPrice < Config.MinimumFuelPrice then QBCore.Functions.Notify(locale("station_price_too_low"), 'error', 7500) return end
                    if NewFuelPrice > Config.MaxFuelPrice then
                        QBCore.Functions.Notify(locale("station_price_too_high"), "error")
                    else
                ...
                local fuelprice = exports['qb-input']:ShowInput({
                    header = locale("input_alter_fuel_price_header_1")..StationFuelPrice..locale("input_alter_fuel_price_header_2"),
                    submitText = locale("input_alter_fuel_price_submit_text"),
                    inputs = { {
                        type = 'number',
                        isRequired = true,
                        name = 'price',
                        text = locale("input_alter_fuel_price_submit_text")
                    }}
                })
                if fuelprice then
                    if Config.FuelDebug then print("Attempting to change fuel price!") end
                    Wait(100)
                    if not fuelprice.price then QBCore.Functions.Notify(locale("station_amount_invalid"), 'error', 7500) return end
    ...
    RegisterNetEvent('cdn-fuel:stations:client:sellstation:menu', function(data) -- Menu, seen after selecting the Sell this Location option.
        ...
        if CanOpen then
            ...
            if Config.Ox.Menu then
                ...
                exports['qb-menu']:openMenu({
                    {
                        header = locale("menu_sell_station_header")..Config.GasStations[location].label,
                        isMenuHeader = true,
                        icon = "fas fa-gas-pump",
                    },
                    {
                        header = locale("menu_sell_station_header_accept"),
                        txt = locale("menu_sell_station_footer_accept")..SalePrice..".",
                        icon = "fas fa-usd",
                        params = {
                            event = "cdn-fuel:stations:client:sellstation",
                            args = {
                    ...
                    {
                        header = locale("menu_header_close"),
                        txt = locale("menu_sell_station_footer_close"),
                        icon = "fas fa-times-circle",
                        params = {
                            event = "qb-menu:closeMenu",
                        }
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if NewName then
                    ...
                    if not NewNameName then QBCore.Functions.Notify(locale("station_name_invalid"), 'error', 7500) return end
                    NewName = NewNameName
                    if type(NewName) ~= "string" then QBCore.Functions.Notify(locale("station_name_invalid"), 'error') return end
                    if Config.ProfanityList[NewName] then QBCore.Functions.Notify(locale("station_name_invalid"), 'error', 7500)
                ...
                local NewName = exports['qb-input']:ShowInput({
                    header = locale("input_change_name_header_1")..Config.GasStations[CurrentLocation].label..locale("input_change_name_header_2"),
                    submitText = locale("input_change_name_submit_text"),
                    inputs = { {
                        type = 'text',
                        isRequired = true,
                        name = 'newname',
                        text = locale("input_change_name_text")
                    }}
                })
                if NewName then
                    if Config.FuelDebug then print("Attempting to alter stations name!") end
                    if not NewName.newname then QBCore.Functions.Notify(locale("station_name_invalid"), 'error', 7500) return end
    ...
    RegisterNetEvent('cdn-fuel:stations:client:managemenu', function(location) -- Menu, seen after selecting the Manage this Location Option.
        ...
        if CanOpen then
            ...
            if Config.Ox.Menu then
                ...
                exports['qb-menu']:openMenu({
                    {
                        header = locale("menu_manage_header")..Config.GasStations[location].label,
                        isMenuHeader = true,
                        icon = "fas fa-gas-pump",
                    },
                    {
                        header = locale("menu_manage_reserves_header"),
                        icon = "fas fa-info-circle",
                        isMenuHeader = true,
                        txt = ReserveLevels..locale("menu_manage_reserves_footer_1")..Config.MaxFuelReserves..locale("menu_manage_reserves_footer_2"),
                    },
                    {
                        header = locale("menu_manage_purchase_reserves_header"),
                        icon = "fas fa-usd",
                        txt = locale("menu_manage_purchase_reserves_footer")..Config.FuelReservesPrice..locale("menu_manage_purchase_reserves_footer_2") ,
                        params = {
                            event = "cdn-fuel:stations:client:purchasereserves",
                            args = {
                                location = location,
                    ...
                    {
                        header = locale("menu_alter_fuel_price_header"),
                        icon = "fas fa-usd",
                        txt = "I want to change the price of fuel at my Gas Station! <br> Currently, it is $"..StationFuelPrice..locale("input_alter_fuel_price_header_2") ,
                        params = {
                            event = "cdn-fuel:stations:client:changefuelprice",
                            args = {
                                location = location,
                    ...
                    {
                        header = locale("menu_manage_company_funds_header"),
                        icon = "fas fa-usd",
                        txt = locale("menu_manage_company_funds_footer"),
                        params = {
                            event = "cdn-fuel:stations:client:managefunds",
                        },
                    },
                    {
                        header = locale("menu_manage_change_name_header"),
                        icon = "fas fa-pen",
                        txt = locale("menu_manage_change_name_footer"),
                        disabled = not Config.GasStationNameChanges,
                        params = {
                            event = "cdn-fuel:stations:client:changestationname",
                        },
                    ...
                    {
                        header = locale("menu_sell_station_header_accept"),
                        txt = locale("menu_manage_sell_station_footer")..math.percent(Config.GasStationSellPercentage, GasStationCost),
                        icon = "fas fa-usd",
                        params = {
                            event = "cdn-fuel:stations:client:sellstation:menu",
                            args = {
    ...
    RegisterNetEvent('cdn-fuel:stations:client:managefunds', function(location) -- Menu, seen after selecting the Manage this Location Option.
        ...
        if CanOpen then
            ...
            if Config.Ox.Menu then
                ...
                exports['qb-menu']:openMenu({
                    {
                        header = locale("menu_manage_company_funds_header_2")..Config.GasStations[CurrentLocation].label,
                        isMenuHeader = true,
                        icon = "fas fa-gas-pump",
                    },
                    {
                        header = locale("menu_manage_company_funds_withdraw_header"),
                        icon = "fas fa-arrow-left",
                        txt = locale("menu_manage_company_funds_withdraw_footer"),
                        params = {
                            event = "cdn-fuel:stations:client:WithdrawFunds",
                            args = {
                                location = location,
                    ...
                    {
                        header = locale("menu_manage_company_funds_deposit_header"),
                        icon = "fas fa-arrow-right",
                        txt = locale("menu_manage_company_funds_deposit_footer"),
                        params = {
                            event = "cdn-fuel:stations:client:DepositFunds",
                            args = {
                                location = location,
                    ...
                    {
                        header = locale("menu_manage_company_funds_return_header"),
                        txt = locale("menu_manage_company_funds_return_footer"),
                        icon = "fas fa-circle-left",
                        params = {
                            event = "cdn-fuel:stations:client:managemenu",
                            args = {
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if Withdraw then
                    ...
                    if not WithdrawAmounts then QBCore.Functions.Notify(locale("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then QBCore.Functions.Notify(locale("station_withdraw_too_little"), 'error', 7500) return end
                    if amount > StationBalance then QBCore.Functions.Notify(locale("station_withdraw_too_much"), 'error', 7500) return end
                    WithdrawAmount = tonumber(amount)
                    if (StationBalance - WithdrawAmount) < 0 then
                        QBCore.Functions.Notify(locale("station_withdraw_too_much"), 'error', 7500)
                    else
                ...
                local Withdraw = exports['qb-input']:ShowInput({
                    header = locale("input_withdraw_funds_header") ..StationBalance,
                    submitText = locale("input_withdraw_submit_text"),
                    inputs = { {
                        type = 'number',
                        isRequired = true,
                        name = 'amount',
                        text = locale("input_withdraw_text")
                    }}
                })
                if Withdraw then
                    if Config.FuelDebug then print("Attempting to Withdraw!") end
                    ...
                    if not Withdraw.amount then QBCore.Functions.Notify(locale("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then QBCore.Functions.Notify(locale("station_withdraw_too_little"), 'error', 7500) return end
                    if amount > StationBalance then QBCore.Functions.Notify(locale("station_withdraw_too_much"), 'error', 7500) return end
                    WithdrawAmount = tonumber(amount)
                    if (StationBalance - WithdrawAmount) < 0 then
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if Deposit then
                    ...
                    if not DepositAmounts then QBCore.Functions.Notify(locale("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then QBCore.Functions.Notify(locale("station_deposit_too_little"), 'error', 7500) return end
                    DepositAmount = tonumber(amount)
                    if (DepositAmount) > bankmoney then
                        QBCore.Functions.Notify(locale("station_deposity_too_much"), "error")
                    else
                ...
                local Deposit = exports['qb-input']:ShowInput({
                    header = locale("input_deposit_funds_header") ..StationBalance,
                    submitText = locale("input_deposit_submit_text"),
                    inputs = { {
                        type = 'number',
                        isRequired = true,
                        name = 'amount',
                        text = locale("input_deposit_text")
                    }}
                })
                if Deposit then
                    if Config.FuelDebug then print("Attempting to Deposit!") end
                    ...
                    if not Deposit.amount then QBCore.Functions.Notify(locale("station_amount_invalid"), 'error', 7500) return end
                    if amount < 1 then QBCore.Functions.Notify(locale("station_deposit_too_little"), 'error', 7500) return end
                    DepositAmount = tonumber(amount)
                    if (DepositAmount) > bankmoney then
                        QBCore.Functions.Notify(locale("station_deposity_too_much"), "error")
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasemenu', function(location) -- Menu, seen after selecting the purchase this location option.
        ...
        if bankmoney < costofstation then
            QBCore.Functions.Notify(locale("not_enough_money_in_bank").." $"..costofstation, 'error', 7500) return
        end
        ...
        if Config.Ox.Menu then
            ...
            exports['qb-menu']:openMenu({
                ...
                },
                {
                    ...
                    icon = "fas fa-info-circle",
                    isMenuHeader = true,
                    txt = locale("menu_purchase_station_header_1")..costofstation..locale("menu_purchase_station_header_2"),
                },
                {
                    header = locale("menu_purchase_station_confirm_header"),
                    icon = "fas fa-check-circle",
                    txt = locale("menu_purchase_station_confirm_footer")..costofstation..'!' ,
                    params = {
                        event = "cdn-fuel:stations:client:purchaselocation",
                        args = {
                            location = location,
                ...
                {
                    header = locale("menu_header_close"),
                    txt = locale("menu_purchase_station_cancel_footer"),
                    icon = "fas fa-times-circle",
                    params = {
                        event = "qb-menu:closeMenu",
                    }
    ...
    RegisterNetEvent('cdn-fuel:stations:openmenu', function() -- Menu #1, the first menu you see.
        ...
        if Config.Ox.Menu then
            ...
            exports['qb-menu']:openMenu({
                ...
                {
                    header = locale("menu_ped_manage_location_header"),
                    txt = locale("menu_ped_manage_location_footer"),
                    icon = "fas fa-usd",
                    params = {
                        event = "cdn-fuel:stations:client:managemenu",
                        args = CurrentLocation,
                ...
                {
                    header = locale("menu_ped_purchase_location_header"),
                    txt = locale("menu_ped_purchase_location_footer"),
                    icon = "fas fa-usd",
                    params = {
                        event = "cdn-fuel:stations:client:purchasemenu",
                        args = CurrentLocation,
                ...
                {
                    header = locale("menu_ped_emergency_shutoff_header"),
                    txt = locale("menu_ped_emergency_shutoff_footer")..PumpState,
                    icon = "fas fa-gas-pump",
                    params = {
                        event = "cdn-fuel:stations:client:Shutoff",
                        args = CurrentLocation,
                ...
                {
                    header = locale("menu_ped_close_header"),
                    txt = locale("menu_ped_close_footer"),
                    icon = "fas fa-times-circle",
                    params = {
                        event = "qb-menu:closeMenu",
                    }
